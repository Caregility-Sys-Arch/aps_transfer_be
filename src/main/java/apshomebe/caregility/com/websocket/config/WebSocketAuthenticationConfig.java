...
public class WebSocketAuthenticationConfig implements WebSocketMessageBrokerConfigurer {
	...
	private class MyInboundChannelInterceptor implements ChannelInterceptor {
		...
		public Message<?> preSend(Message<?> message, MessageChannel channel) {
			...
			List<String> apsMachineName = accessor.getNativeHeader("APS_MACHINE_NAME");

			logger.debug("APS_HOST_NAME:{}", apsMachineName);

			String sessionId = accessor.getSessionId();
			logger.debug("Session Id:{}", sessionId);
			// Note: We are losing APS_MACHINE_NAME headers information in
